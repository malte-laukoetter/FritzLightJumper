...
-- CONTRACT
genVisAreas :: Time -> Int -> [Area]

-- PURPOSE
-- Generates a pseudo random area list where the player has to jump over

-- DEFINITION
genVisAreas time seed = [((xPos x, yPos x - size), (xPos x + size, yPos x)) | x <- [cX-2..cX+(5*maxBlo)]] ++ [bott x | x <- [cX-2..cX+(5*maxBlo)]]
  where aX      | time >= 30 = getScrollAlpha $ time - 30
                | otherwise  = 0
        maxBlo  = (floor $ utiDiv time 2500) + 1 
        alpha   = floor $ utiDiv 10 (realToFrac maxBlo)
        cX      = floor $ utiDiv aX (realToFrac alpha)
        seedfac = sin (utiDiv seed 8.24234)
        maxGap  | time < 2000 = 7
                | otherwise   = 3
        size    = 2
        ...
        yPos x' | (((tan (utiDiv (3*x') 7.23234))-0.4) + seedfac) > 0.2 = 9-size
                | (((tan (utiDiv (2*x') 3.34344))-0.5) - seedfac) > 0   = 12-size
                | otherwise                                 = 12
        bott x' | (sin $ utiDiv x' 9) > 0 || time < 40 || ((tan $ utiDiv x' 24) + seedfac) > 0.1  = ((x'*alpha, 11), (x'*alpha+alpha, 12))
                | otherwise                                                                       = ((0,0), (0,0))
